...
| 语义 | 主色 | 浅底 | 边框 | 文字 |
|---|---|---|---|---|
| 成功 / 已采纳 / 保留 | `#16a34a` | `#dcfce7` (`#f0fdf4` 更浅) | `#bbf7d0` | `#15803d` |
| 警告 / 进行中 / 可改 | `#a16207` / `#1d4ed8` | `#fef3c7` / `#eff6ff` | `#fde68a` / `#bfdbfe` | `#a16207` / `#1d4ed8` |
| 危险 / 拒绝 / 禁止 | `#dc2626` | `#fef2f2` | `#fecaca` | `#b91c1c` |
| 信息（语义） | `#0ea5e9` | `#dbeafe` | `#bfdbfe` | `#1d4ed8` |

**复刻边界三色（项目专属）** —— 在 `breakdown-types.ts` 的 `BOUNDARY_META`：
- `keep`（必须保留）：bg `#f0fdf4` / border `#bbf7d0` / text `#15803d` / dot `#16a34a`
- `change`（可以改）：bg `#eff6ff` / border `#bfdbfe` / text `#1d4ed8` / dot `#2563eb`
- `ban`（禁止复制）：bg `#fef2f2` / border `#fecaca` / text `#b91c1c` / dot `#dc2626`
...
**状态徽章**（图片左上角，缩略图覆盖）
```tsx
"absolute top-2 left-2 inline-flex items-center gap-1 h-5 px-1.5 rounded-md text-[10.5px] font-extrabold"
// 成功：bg-[#16a34a] text-white
// 失败：bg-[#dc2626] text-white
// 中性：bg-white/95 text-[#15803d] shadow-sm
```
...
- 完成 `#16a34a`，进行中 `var(--near-black)`，未启动 `rgba(0,0,0,0.08)`
...
- 句号在短文案末尾
- 硬编码 `#xxxxxx` 替代 token（除业务专属色板如 lifecycle / boundary）
